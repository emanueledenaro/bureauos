...
Local-first kernel primitives, registries, policy gates, run lifecycle, reports, GitHub signal ingestion, provider-aware agent drafting, OpenCode-style provider connector config, model capability/budget route controls, capability registry exposure, and internal operational signal triggers are implemented. Runtime execution adapters and deeper external connectors are still being wired behind policy.
