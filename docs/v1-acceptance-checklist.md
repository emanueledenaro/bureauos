...
| Search and promotion | Important memory can be retrieved predictably; background consolidation and semantic indexing are not required for v1. | partial | markdown search, SQLite FTS5 index honoring configured `search_index` + `bureau memory index status\|rebuild`, optional offline TF-IDF semantic index (`provider: local`), memory packet assembly |
