...
| Security baseline | Secrets are not stored in repo; destructive shell/file/git actions are blocked by default; provider auth is kept in local workspace auth files. | partial | runtime boundary, auth store, security docs |
