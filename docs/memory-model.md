...
The Supreme Coordinator needs an audited portfolio view across all project teams.

The implemented cross-project report is generated by `BusinessReportService` as a `cross-project-executive-report` artifact. It combines:

- project records
- per-project `OWNERSHIP.md`
- client records
- opportunities
- pending approvals
- active runs

The report lists each project with PM, team, client, status, active runs, pending approvals, pipeline value by client, risk state, and next executive action. Project teams still remain scoped to their own memory; this cross-project comparison is an executive-only operating view.
